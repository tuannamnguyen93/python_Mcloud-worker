...
cdef int py_worker_data_clb_wrapper(mcloud_h.MCloud* mcloud, mcloud_h.MCloudPacket*packet, void* python_callback) :
        ...
        result = python_data_callback(callback,sample,sampleA, <void*>python_callback)



        cdef s2stime.S2S_Time startT
        cdef s2stime.S2S_Time stopT
        cdef s2stime.S2S_Time tmpT
        cdef char     startTime[128]
        cdef char     stopTime[128]
        cdef int n = 1
        cdef int i
        cdef unsigned int startMS =0
        cdef unsigned int stopMS =0
        cdef int windex = 0
        cdef MCloudWordToken* tokenA = NULL

        tokenA = mcloudWordTokenArrayCreate (n)
        ...
        for i in range(0,n):
            ...
            windex = windex + 1
            tokenA[i].index      = windex
            tokenA[i].internal   = strdup("internalForm")
            tokenA[i].written    = strdup(result)
            tokenA[i].spoken     = strdup("seeeee")
            tokenA[i].confidence = 1.0
            tokenA[i].startTime  = startMS
            tokenA[i].stopTime   = stopMS
            tokenA[i].isFiller   = 0



        s2stime.s2s_AddToTime (&startT, tokenA[0].startTime, 0)
        s2stime.s2s_AddToTime (&stopT,  tokenA[n-1].stopTime, 0)
        timePrint (&startT, startTime)
        timePrint (&stopT,  stopTime)
        cdef MCloudPacket* npm = NULL
        npm = <MCloudPacket*> mcloudPacketInitFromWordTokenA (mcloud, startTime, stopTime, startMS, stopMS, NULL, tokenA, n)
