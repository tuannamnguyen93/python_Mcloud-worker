...
def init_callback():
    if os.path.exists(TEMP_FILE):
        subprocess.call(["rm -rf %s" % TEMP_FILE], shell=True)
    global proc
...
def clean(tempDir):
    fin = open("%s/recording.adc.en-EN.fgets" % tempDir, 'r')
    for line in fin:
        if line.startswith('#'): line = '/' + line[1:]
        audioFile = line.split()[0]
        subprocess.call(["touch %s-ready" % audioFile], shell=True)


serverHost = "mediator.pervoice.com"
serverPort = 60021
inputFingerPrint  = "en-DB"
inputType         = "audio"
outputFingerPrint = "en-DB"
outputType        = "text"
star_time         = ""
specifier           = ""
stream_id = ""
